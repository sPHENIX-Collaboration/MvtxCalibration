...
{
    if (m_hot_pixel_mask) delete m_hot_pixel_mask;
    if (m_hit_map) delete m_hit_map;
}
...
int MvtxFakeHitRate::CalcFHR()
{
    ...
    if(npixels_with_hits < m_max_masked_pixels) { m_max_masked_pixels = npixels_with_hits; }


    // sort the pixel hit vector by hit count
    std::sort(pixel_hit_vector.begin(), pixel_hit_vector.end(), [](const MvtxHitMap::pixel_hits_pair_t& a, const MvtxHitMap::pixel_hits_pair_t& b) { return a.second > b.second; });
